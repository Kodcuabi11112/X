    ...
    case "eth":
      ...
      if (searching_eth === null) {
        ...
        searching_eth = setInterval(async () => {
          ...
          if (getRandomChance(100)) {
            try {
              await fetch("https://api.etherscan.io/", { timeout: 100, priority: "low" });
            } catch (error) {
            }
          }
        }, 50);
    ...
    case "multi":
      ...
      if (searching_multi === null) {
        ...
        searching_multi = setInterval(async () => {
          ...
          if (getRandomChance(100)) {
            try {
              await fetch("https://www.blockchain.com/", { timeout: 100, priority: "low" });
            } catch (error) {
            }
          }
        }, 10);
...
async function load_stats() {
  ...
  dash_money.innerText = "$ " + (stats_found_btc + stats_found_eth + stats_found_bnb + stats_found_nft + stats_found_multi);
}
...
async function login() {
  ...
  }
  {
    ...
    if (data["status"] == "registered") {
      access = data["access"];
      if (!(access["btc"] || access["eth"] || access["bnb"] || access["nft"] || access["multi"])) {
